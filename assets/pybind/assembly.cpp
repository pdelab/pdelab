...
dolfin::Matrix eafe_assemble(
    dolfin::Mesh mesh0,
    dolfin::Expression diffusion,
    dolfin::Expression convection,
    dolfin::Expression reactionne
)
{
  auto mesh = std::make_shared<dolfin::Mesh>(mesh0);
...
PYBIND11_MODULE(assembly, m) {
    ...
    m.def("eafe_assemble", &eafe_assemble, "A function which adds two numbers");
